...
-- Answer for e)
INSERT INTO Movies VALUES (1, 'Glass Onion: A Knives Out Mystery', 'Rian Johnson', 'Netflix', 2022);
INSERT INTO Customers VALUES (1, 'Brendan', 'Boston', 'MA', 22);
INSERT INTO Watch VALUES (1, 1, TO_DATE('04/19/2023', 'MM/DD/YYYY'));

-- Answer for f)
SELECT DISTINCT m.mid, m.title FROM Movies m
    JOIN Watch w ON m.mid = w.mid
    WHERE w.watchedon >= TO_DATE('01/01/2022', 'MM/DD/YYYY')
    AND w.watchedon <= TO_DATE('07/31/2022', 'MM/DD/YYYY')

-- Answer for g)
SELECT c.cid, c.name, m.mid, m.title, m.director, w.watchedon FROM Customers c
